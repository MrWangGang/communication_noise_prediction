...
class LstmTransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        lstm_out = self.lstm_norm(lstm_out[:, -1, :])  # 最后一个时间步 + LayerNorm
        lstm_out_projected = self.lstm_output_projection(lstm_out)

        # Transformer 分支
        transformer_input = self.transformer_input_projection(x)
        transformer_out = self.transformer_encoder(transformer_input)
        transformer_out = self.transformer_norm(transformer_out[:, -1, :])  # 最后一个时间步 + LayerNorm
...
def calculate_metrics(y_true, y_pred):
    ...
    # 避免除以零，处理 std_y_true 或 std_y_pred 为零的情况
